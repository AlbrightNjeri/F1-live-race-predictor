...
with st.sidebar:
    ...
    if not demo_mode:
        year         = st.number_input("Season", min_value=2018, max_value=2030, value=2026)
        ...
        if st.button("Start Live Polling"):
            ...
            st.session_state["live_config"] = f"{int(year)} {gp} {session_type}"
            st.success(f"Polling {int(year)} {gp} {session_type} — data loads in ~30s")
        if st.button("Stop Polling"):
            if st.session_state.poller:
                st.session_state.poller.stop()
                st.session_state.poller = None
                st.session_state.pop("live_config", None)
        if "live_config" in st.session_state:
            st.caption(f"Active: {st.session_state['live_config']}")
        poller_ref = st.session_state.poller
        if poller_ref and poller_ref.latest_state:
            st.caption(f"Last data: {poller_ref.latest_state.last_updated.strftime('%H:%M:%S')}")
        elif poller_ref:
            st.info("Poller running — waiting for first data load (~30s)...")
    else:
        demo_gp   = st.text_input("GP name", value="Monaco")
        demo_year = int(st.number_input("Year", min_value=2018, max_value=2030, value=2026))
...
def get_state() -> RaceState | None:
    if demo_mode:
        ...
        return base
    # Live mode — read from poller thread
    poller = st.session_state.get("poller")
    if poller is None:
        return None
    s = poller.latest_state
    if s is None:
        return None
    # Always return a fresh copy with updated timestamp so Streamlit sees a change
    s.last_updated = s.last_updated  # reference kept
    return s


state = get_state()

# Debug info — shown only in live mode when waiting
if not demo_mode and state is None and st.session_state.get("poller"):
    st.info("Poller is running — FastF1 is fetching data. This takes 20–40 seconds on first load. The page will update automatically.")
    elapsed_placeholder = st.empty()
    import time as _time
    elapsed_placeholder.caption(f"Poller started. Current time: {__import__('datetime').datetime.utcnow().strftime('%H:%M:%S UTC')}")
